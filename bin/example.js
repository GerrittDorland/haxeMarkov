...
MainIntermediate.loadMap = function(myMap) {
	var fin = js_node_Fs.readFileSync("database.txt",{ encoding : "utf8"});
	var key1Delim;
	var key2Delim;
	var wordDelim;
	key1Delim = js_Boot.__cast(255 , haxe_Utf8);
	key2Delim = js_Boot.__cast(254 , haxe_Utf8);
	wordDelim = js_Boot.__cast(253 , haxe_Utf8);
	while(0 <= fin.length - 5) {
		var subString = MainIntermediate.readLine(0,fin);
		var key1 = subString.split(js_Boot.__cast(key1Delim , String));
		var key2 = subString.split(js_Boot.__cast(key2Delim , String));
		var words = subString.split(js_Boot.__cast(wordDelim , String));
		var key = key1[0] + key2[0];
		if(__map_reserved[key] != null) {
			myMap.setReserved(key,words);
		} else {
			myMap.h[key] = words;
		}
	}
};
MainIntermediate.readLine = function(index,data) {
	var it = index;
	var itStart = index;
...
de_polygonal_core_math_random_ParkMiller.prototype = $extend(de_polygonal_core_math_random_RNG.prototype,{
	...
	,__class__: de_polygonal_core_math_random_ParkMiller
});
